...
window.WORKS = {
  ...
  /* ======================= 포트폴리오 ======================= */
  portfolio: [
    { slug:"pf-site-eeecs", cat:"웹 · 서비스", order:25, title:"EEECS 국제학회 웹사이트", period:"2022–2024", org:"IC-EEECS (오사카)", cover:"assets/img/portfolio/site-eeecs.jpg", tags:["웹","학회"], links:{ url:"https://ic-eeecs.org" }, desc:["전기·전자·컴퓨터공학 국제학회(EEECS) 공식 웹사이트 제작. 등록·결제 안내, 논문 투고 절차, 프로그램 페이지 포함."] },
    { slug:"pf-site-ctkorea", cat:"웹 · 서비스", order:21, title:"한국문화콘텐츠기술학회(KOCTA) 웹사이트", period:"2024", org:"ctkorea.org", cover:"assets/img/portfolio/site-ctkorea.jpg", tags:["웹","학회"], links:{ url:"https://ctkorea.org" }, desc:["한국문화콘텐츠기술학회 공식 웹사이트 제작 — 논문 투고·학술대회·회원 자료 제공."] },
    { slug:"pf-site-crc", cat:"웹 · 서비스", order:20, title:"KAIST 융복합연구센터 웹사이트", period:"2024", org:"crc.kaist.ac.kr", cover:"assets/img/portfolio/site-crc-kaist.jpg", tags:["웹","KAIST"], links:{ url:"https://crc.kaist.ac.kr" }, desc:["KAIST 공과대학 융복합연구센터 공식 웹사이트 제작."] },
    { slug:"pf-site-daeseungpark", cat:"웹 · 서비스", order:26, title:"개인 리서치 저널 daeseungpark.com", period:"2025", org:"daeseungpark.com", cover:"assets/img/portfolio/site-daeseungpark.jpg", tags:["웹","저널"], links:{ url:"https://daeseungpark.com", repo:"https://github.com/UniverseMaker/universemaker.github.io" }, desc:["생성형 AI·시계열 예측·대화형 시스템 연구를 기록하는 개인 리서치 저널 사이트."] },
    ...
    { slug: "pf-khnp-forecast", cat: "AI · R&D", order: 24,
      title: "원전 발전량 예측 (한수원)",
    ...
    { slug: "pf-drone-cv-damage", cat: "AI · R&D", order: 7,
      ...
      tags: ["컴퓨터비전", "드론"],
      links: { url: "https://github.com/UniverseMaker/Damaged-Pixel-Analysis-Server", preprocessing: "https://github.com/UniverseMaker/Damaged-Pixel-Analysis-Preprocessing-Software" },
    ...
    { slug: "pf-conference-payment", cat: "웹 · 서비스", order: 19,
      title: "학회 결제 시스템",
    ...
    { slug: "pf-paper-submission", cat: "웹 · 서비스", order: 18,
      title: "논문 투고 시스템",
    ...
    { slug:"pf-firefighter-sim", cat:"시뮬레이션 · XR", order:23,
    ...
    { slug:"pf-eyedrive", cat:"도구 · 라이브러리", order:22,
    ...
    { slug:"pf-indoor-nav-frontend", cat:"실내측위 · 내비", order:17,
    ...
    { slug:"pf-indoor-route-php", cat:"실내측위 · 내비", order:16,
    ...
    { slug:"pf-geojson-nodenet", cat:"실내측위 · 내비", order:15,
    ...
    { slug:"pf-medical-compress-fw", cat:"시스템 · 임베디드", order:14,
    ...
    { slug:"pf-seq2seq-watertemp", cat:"AI · R&D", order:13,
      ...
      links:{ url:"https://github.com/UniverseMaker/Seq2Seq-Water-Temperature-Prediction-using-Multivariate-Model", correlation:"https://github.com/UniverseMaker/Correlation-Analysis-for-Multivariate-Deep-Learning-Model" },
      desc:["다변량 입력을 사용하는 Seq2Seq(인코더–디코더) 시계열 모델로 해수 수온을 예측한다. ICTC 2023 발표 논문의 Python·TensorFlow 구현.",
             "같은 연구에서 입력 변수를 고르기 위해 만든 다변량 상관분석 코드를 별도 저장소로 함께 공개했다. 부이 관측 변수(풍속·풍향·기압·습도·기온·파고·파주기·파향 등)를 정규화한 뒤 수온을 기준으로 상관계수 행렬을 뽑아 히트맵으로 그리고, 랜덤포레스트 회귀·분류로 변수 중요도를 산출해 어떤 변수를 Seq2Seq 입력에 넣을지 판단하는 데 썼다. 예측 모델의 핵심 코드는 담지 않은 분석 전용 스크립트다."] },
    { slug:"pf-coding-textutil", cat:"도구 · 라이브러리", order:12,
    ...
    /* ---- 신규(2026-08-31): Red-Eye-Engine (GitHub 공개 2023-08-08) ---- */
    { slug:"pf-redeye-engine", cat:"도구 · 라이브러리", order:11,
      title:"Red-Eye-Engine — HTTP 통신 처리 올인원 C# 라이브러리",
      period:"2023", org:"GitHub · C#/.NET Framework 3.5 (DLL)",
      cover:"assets/portfolio/pf-redeye-engine/cover.svg",
      tags:["HTTP","라이브러리","C#"],
      links:{ url:"https://github.com/UniverseMaker/Red-Eye-Engine", store:"https://github.com/UniverseMaker/Red-Eye-Engine/releases/tag/publish" },
      files:[{ name:"Red-Eye-Engine 라이브러리 ZIP 직접 다운로드 (20230808 Version 1)", url:"https://github.com/UniverseMaker/Red-Eye-Engine/releases/download/publish/20230808.Red.Eye.Engine.HTTP.Library.zip" }],
      desc:["HTTP 요청·응답 처리를 한 벌로 묶어 동적 링크 라이브러리(DLL)로 배포하는 C# 클래스 라이브러리. 프록시·타임아웃·임의 헤더·gzip/deflate 자동 해제·쿠키 조립을 감싼 전송 함수군을 중심에 두고, 그 위에 이미지 보정·네트워크 어댑터 제어·ADB 연동·설정 파일 입출력 같은 주변 기능을 같은 어셈블리에 모았다.",
             "핵심인 Engine 클래스가 HttpSend·HttpSendBytes·HttpSendSecureCookie·HttpSendSocket 네 갈래 전송 경로를 제공하고, 여기에 URL 인코딩/디코딩·MD5 해시·쿠키 병합·한글 자소 분해·조립 같은 문자열 유틸리티가 붙는다. .NET Framework 3.5를 대상으로 하는 클래스 라이브러리(OutputType=Library, AssemblyName=RedEyeEngine)이며, 2023년 8월 8일 6개 커밋으로 공개된 뒤 그대로 남아 있다."] },
    ...
    /* ---- 신규(2026-08-28): AddressKG · 도시안전 AI API · ulogin · LLM Lab ---- */
    { slug:"pf-addresskg", cat:"AI · R&D", order:37,
      title:"AddressKG — 국가주소정보 지식그래프",
    ...
    { slug:"pf-citysafety-ai-api", cat:"AI · R&D", order:36,
      title:"도시안전 AI API — 재난 상황 인지 멀티모달 모델 서빙 소프트웨어",
      period:"2026", org:"KAIST · 지역혁신클러스터육성(R&D) 도시안전 AI",
      cover:"assets/portfolio/pf-citysafety-ai-api/cover.svg",
      tags:["멀티모달 LLM","재난안전","FastAPI","KAIST"],
      figs:[{src:"assets/portfolio/pf-citysafety-ai-api/fig-architecture.svg", cap:"시스템 아키텍처 — 모달리티 인코더 4종에서 프로젝션·LLM 백본을 거쳐 교체 가능한 서빙 백엔드와 API까지"},{src:"assets/portfolio/pf-citysafety-ai-api/fig-pipeline.svg", cap:"재난 상황 인지 종단간 파이프라인 — 센서 입력·검색 근거·공간 매핑·추론·플랫폼 연계까지 8단계를 한 번에 관통"}],
      desc:["영상·센서·공간정보·텍스트 4개 모달리티를 하나의 LLM 백본 위에서 함께 읽어 화재·침수·붕괴 상황을 인지하고, 그 결과를 기관 간 표준 스키마로 주고받는 도시안전 AI 추론 서빙 소프트웨어.",
             "KAIST가 전담하는 멀티모달 파운데이션 모델의 학습·서빙·평가·연계 규격을 하나의 코드베이스에 담았다. FastAPI 게이트웨이가 추론과 종단간 파이프라인을 REST·서버전송이벤트로 노출하고, 모든 기관 간 경계는 JSON Schema 단일 진실원본으로 검증된다. 추론 백엔드는 문자열 하나로 교체되어 GPU와 실모델 가중치 없이도 API 경로·지연 측정·리포트를 그대로 실측할 수 있다."] },
    { slug:"pf-universal-login", cat:"웹 · 서비스", order:35,
      title:"ulogin — 로컬 앱의 API 키를 걷어내는 범용 로그인·SSO 서버",
      period:"2026", org:"GitHub · PHP 7.2+ / MySQL",
      cover:"assets/portfolio/pf-universal-login/screenshot-demo-after-login.png",
      tags:["SSO","인증","PHP","MySQL"],
      links:{ url:"https://github.com/UniverseMaker/Universal-Login-SSO-Server" },
      figs:[{src:"assets/portfolio/pf-universal-login/01-sso-flow.svg", cap:"리다이렉트 SSO 3단계 — validate → authorize → exchange. 리다이렉트 URL에는 60초짜리 1회용 code만 실리고, 시크릿은 POST 본문으로만 나간다"},{src:"assets/portfolio/pf-universal-login/02-architecture.svg", cap:"시스템 구조 — 모든 요청이 core/bootstrap.php 한 곳을 지나고, 나머지 12개 모듈은 부트 상수가 없으면 404로 즉시 종료한다"},{src:"assets/portfolio/pf-universal-login/screenshot-login-sso.png", cap:"SSO 모드 로그인 — 헤더에 요청한 서비스명이 표시되고 로그인 후 그 앱으로 되돌아간다 (실제 설치본 촬영, 등장 데이터는 전부 가상)"},{src:"assets/portfolio/pf-universal-login/screenshot-demo-after-login.png", cap:"로그인 후 로컬 AI 앱 — 3축 등급 배지·포인트와 서버가 내려준 payload. 키는 하드코딩이 아니라 로그인 응답에서 오고, 앞 6자만 표시되며 메모리에만 머문다"},{src:"assets/portfolio/pf-universal-login/screenshot-install-env-check.png", cap:"웹 설치기 1단계 환경 점검 — PHP 7.2 이상·PDO MySQL·OpenSSL·mbstring 등 필수 7항목을 서버에서 직접 검사하고, 하나라도 없으면 진행을 막는다 (실제 설치본 촬영)"},{src:"assets/portfolio/pf-universal-login/screenshot-admin-dashboard.png", cap:"관리자 콘솔 대시보드 — 회원·로그인·포인트·세션 타일과 3축 등급(계정·역할·결제) 분포. 권한 모델이 3축이라는 사실이 통계 화면에 그대로 드러난다 (등장 데이터는 전부 가상)"},{src:"assets/portfolio/pf-universal-login/screenshot-mypage-info.png", cap:"마이페이지 정보 탭 — 3축 등급 배지와 계정 정보. 등급·역할·포인트·아이디·이메일은 이 폼으로 바꿀 수 없어 권한 상승 경로가 막혀 있다 (등장 데이터는 전부 가상)"}],
      desc:["배포된 HTML의 JavaScript는 비밀이 아니다 — 로컬 AI 앱에 API 키를 하드코딩해 남에게 건네는 순간 그 키도 함께 공개되고, 키가 새면 제3자가 호출하고 청구서는 만든 사람에게 온다. ulogin은 앱을 정적 HTML 한 장으로 남겨 둔 채 로그인만 서버로 분리하고, 인증된 사용자에게 등급이 허락하는 시크릿만 돌려주는 범용 로그인·SSO 서버다.",
             "공용 호스팅(DB 1개·SSH 없음·빌드 도구 없음·PHP가 7.2일 수 있음)을 전제로 설계해 Composer도 vendor/도 쓰지 않고 PHP 내장 확장(PDO·OpenSSL·mbstring·json)만 사용한다. 폴더째 올리고 브라우저로 웹 설치기를 8단계 따라가면 끝난다. PHP 7.2+ / MySQL·MariaDB, API 엔드포인트 32파일(메서드×경로 43), 테이블 15개, 화면 8개, 에러 코드 31종. 보안 감사 4회차·QA 3회차를 거쳐 미해결 Critical·High 0건으로 MIT 라이선스 공개 저장소에 올렸다."] },
    { slug:"pf-chatbot-platform", cat:"AI · R&D", order:34,
      title:"LLM Lab — 범용 LLM 테스트·연구 플랫폼",
      period:"2026", org:"GitHub · Vanilla JS · PHP/Python",
      cover:"assets/portfolio/pf-chatbot-platform/cover.svg",
      tags:["LLM","RAG","에이전트","오픈소스"],
      links:{ url:"https://github.com/UniverseMaker/Universal-LLM-Test-Research-Platform", demo:"https://ai.daeseungpark.com/chat/" },
      figs:[{src:"assets/portfolio/pf-chatbot-platform/architecture.svg", cap:"플랫폼 아키텍처 — 브라우저 엔진, 선택적 릴레이(server.py·proxy.php), 외부 OpenAI 호환 엔드포인트와 데이터베이스"},{src:"assets/portfolio/pf-chatbot-platform/profile-pipeline.svg", cap:"연결 프로필 JSON에서 요청 조립·릴레이 폴백·스트리밍 수신을 거쳐 메시지별 인스펙터에 이르는 파이프라인"}],
      desc:["특정 모델이나 벤더에 묶이지 않고, OpenAI 호환 API를 제공하는 어떤 LLM 서버든 연결 프로필 JSON 한 장으로 붙여 곧바로 시험할 수 있게 만든 순수 프런트엔드 연구 플랫폼이다. 빌드 도구도 프레임워크도 없이 index.html을 열면 그대로 실행되며, API 키와 연결 정보는 서버로 올라가지 않고 사용자의 브라우저 저장소에만 남는다.",
             "하나의 공용 커널 위에 Chat · RAG Lab · Chain · Agent/Tools · Eval/Bench · Simulate 여섯 개의 워크벤치를 얹어, 같은 연결·같은 파라미터로 대화·검색증강생성·프롬프트 체이닝·툴 호출·A/B 평가·모델 대 모델 시뮬레이션을 한자리에서 비교한다. 모든 요청은 인스펙터에 URL·헤더·바디·응답·지연·토큰 사용량으로 그대로 펼쳐지고, 같은 요청을 curl·python·fetch 코드로 복제할 수 있다. 2026년 7월 단일 모델 챗봇 웹앱으로 시작해 8월 범용 플랫폼으로 방향을 틀었고, MIT 라이선스 오픈소스로 공개했다."] }
  ],
